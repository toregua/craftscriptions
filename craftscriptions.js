...
const dogecore = require('./bitcore-lib-craftcoin');
...
async function main() {
    ...
    } else if (cmd === 'mint-craftmap') {
        await mintCraftmap();
    } else if (cmd === 'deploy-crc') {
        await deployCrc();
    } else if (cmd === 'deploy-crc20') {
        await luck20Deploy();
    } else if (cmd === 'mint-crc20') {
        await mintcrc20();
    } else if (cmd === 'wallet') {
        await wallet();
    } else if (cmd === 'server') {
...
async function walletSync() {
    ...
    console.log('Syncing UTXOs with local Craftcoin node via RPC');
...
async function walletSync2() {
    ...
    let response = await axios.get(`https://craftcoin.minepixel.io/api/address/${wallet.address}`);
    let response1 = await axios.get(`https://craftcoin.minepixel.io/api/address/${wallet.address}/txs`);
...
async function mintCraftmap() {
    ...
    for (let i = start; i <= end; i++) {
        const data = Buffer.from(`${i}.craftmap`, 'utf8');
        const contentType = 'text/plain';

        let wallet = JSON.parse(fs.readFileSync(WALLET_PATH));
        let txs = inscribe(wallet, address, contentType, data);
        console.log(`${i}.craftmap`);
...
function inscribe(wallet, address, contentType, data) {
    ...
    console.log("Generated Transaction Hex:", txs.map(tx => tx.toString()));
...
async function broadcast(tx, retry) {
    ...
    const curlCommand = `curl --ssl-no-revoke -X POST -H "Content-Type: text/plain" --data-binary "${txHex}" "https://craftcoin.minepixel.io/api/tx"`;
...
async function extract(txid) {
    let resp = await axios.get(`https://craftcoin.minepixel.io/api/tx/${txid}`);
    ...
    while (remaining && chunks.length) {
        ...
        if (n !== remaining - 1) {
            ...
            resp = await axios.get(`https://craftcoin.minepixel.io/api/tx/${txid}`);
...
async function luck20Deploy() {
    ...
    // Ensure the WALLET_PATH is used to read the wallet file
    let wallet = JSON.parse(fs.readFileSync(WALLET_PATH));
    let txs = inscribe(wallet, argAddress, contentType, data);
    console.log(`Deploy CRC - ${luck20Tx.tick}`);
    await broadcastAll(txs, false);
}
...
async function mintcrc20() {
    ...
    for (let i = 0; i < argRepeat; i++) {
        ...
        let txs = inscribe(wallet, argAddress, contentType, data);
        console.log(`Mint CRC - ${luck20Tx.tick}, Amount: ${luck20Tx.amt}, Repeat: ${i + 1}`);
        try {
            await broadcastAll(txs, false);
        } catch (error) {
